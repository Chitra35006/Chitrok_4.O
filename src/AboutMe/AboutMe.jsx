...
const AboutMe = () => {
  ...
  return (
    <div id="about-section" className={`mb-10 mt-14 w-10/12 mx-auto px-8 rounded-4xl ${theme === "dark" ? "bg-[#032644]" : "bg-slate-100 "}`}>
      ...
      <motion.h2
        className={`text-3xl md:text-4xl  font-extrabold py-4 ${theme === "dark" ? "text-orange-200" : "text-violet-800"}`}
        initial={{ opacity: 0 }}
      ...
      <section className="flex flex-col md:flex-row items-stretch justify-between px-6 py-8 text-black h-auto md:h-[450px] border-purple-400 border-dashed border-2">
        ...
        >
          <p className="md:text-lg font-medium mb-4 text-gray-500">
            Hi! I&apos;m <span className={`font-bold ${theme === "dark" ? "text-orange-200" : "text-violet-900 "}`}>Chitra</span> Rani Das, a passionate{" "}
            <span className="text-orange-400">Frontend Developer</span> focused on learning and exploring new technologies.
          </p>
          <p className={`md:text-lg font-medium mb-4 ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
      ...
      <div className="m-6 pb-8">
        <motion.h3
          className={`text-2xl font-bold mb-4 text-center ${theme === "dark" ? "text-orange-300" : "text-violet-800"}`}
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          transition={{ duration: 1 }}
        >
